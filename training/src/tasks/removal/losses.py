...
class CombinedLoss(nn.Module):
    ...
    def forward(self,
        ...
        need_mask_interior = (
            self.w_l1_masked > 0
            or self.w_color_moment > 0
            or self.w_saturation > 0
        )
        if need_mask_interior:
            # Removes the outer ring so interior-only losses focus on clearly
            # interior pixels where the watermark signal is above JPEG noise.
        ...
        perc_loss = (self.perceptual(pred, target)
                     if (self.perceptual is not None and self.w_perceptual > 0 and use_perceptual)
