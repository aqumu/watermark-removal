...
def seg_loss(logits: torch.Tensor, target: torch.Tensor,
    ...
    MS (multi-scale) pools pred & target at 4Р“вЂ”/8Р“вЂ”/16Р“вЂ” and computes L1.
...
class SegTrainer:
    def __init__(self, model: nn.Module, cfg: dict,
        ...
        self.grad_accum_steps = max(1, tc.get("grad_accum_steps", 1))
    ...
    def _train_epoch(self, epoch: int) -> float:
        ...
        accum_breakdown: dict | None = None
        ...
        for batch in self.train_loader:
            ...
            # accumulate breakdown for logging
            if accum_breakdown is None:
                accum_breakdown = {k: v for k, v in breakdown.items()}
            else:
                for k in accum_breakdown:
                    accum_breakdown[k] += breakdown[k]
            ...
            if step % self.grad_accum_steps == 0:
                ...
                # average breakdown over accumulated steps
                for k in accum_breakdown:
                    accum_breakdown[k] /= self.grad_accum_steps
                accum_breakdown["grad_norm"] = grad_norm
                accum_breakdown = None
            else:
                grad_norm = 0.0
            ...
            if step % self.log_every == 0:
                ...
                row = {"epoch": epoch, "step": step, "global_step": self._global_step, "lr": f"{lr:.2e}",
    ...
    def _validate(self, epoch: int) -> float:
        ...
        val_row = {"epoch": epoch, "global_step": self._global_step, "iou": f"{avg_iou:.4f}"}
        ...
        if self._sample_dir is not None:
            ...
            if result is not None:
                wm_bgr, pred_mask, gt_mask, loss_map = result
    ...
    # РІвЂќР‚РІвЂќР‚ visualisation sample inference РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚РІвЂќР‚

    @torch.no_grad()
    def _infer_sample(self, size: int = 0):
        """Run inference on the fixed visualisation sample.
        Returns (wm_bgr, pred_mask_f32, gt_mask_f32, weighted_loss_f32) or None on failure.
        """
        import cv2
        import numpy as np
        ...
        # Per-pixel BCE loss heatmap: shows where the model makes the most mistakes.
        # Values are unnormalized floats; _write_image normalizes to max=white.
        eps = 1e-7
        loss_map = -(
            gt_np * np.log(pred_np.clip(eps, 1.0 - eps))
            + (1.0 - gt_np) * np.log((1.0 - pred_np).clip(eps, 1.0 - eps))
        ).astype(np.float32)

        return wm_r, pred_np, gt_np, loss_map
