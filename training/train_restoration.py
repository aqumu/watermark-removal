"""
Train the direct clean-image restoration task.
"""
...
from src.tasks.restoration.dataset import make_splits
from src.tasks.restoration.model import build_model, count_params
from src.tasks.restoration.store_cli import clear_store, load_restoration_cfg, rebuild_store, resolve_store_context
from src.tasks.restoration.trainer import Trainer
...
def load_cfg(path: str) -> dict:
    return load_restoration_cfg(path)
...
def main():
    ...
    candidates = iter_sample_dirs(cfg["dataset"]["root"])
    sample_dir = random.choice(candidates) if candidates else None
    if sample_dir:
        print(f"Visualisation sample: {sample_dir.name}")

    experiment = ExperimentSession(task_name="restoration", cfg=cfg, manifest=run_context.manifest, dashboard=dashboard_sink)
    ...
    def _train_worker(active_trainer):
        try:
